...
export function runPipeline(root: string): PipelineResult {
  const { schema, concepts, edges, symptoms, references, walks, issues } = parseTree(root);
  ...
  issues.push(...validateContent(schema, concepts, edges, symptoms, references, walks));
  ...
  const graph = linkGraph(schema, concepts, edges, symptoms, references, walks, rendered);
