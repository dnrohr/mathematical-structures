...
export function runPipeline(root: string): PipelineResult {
  const { schema, concepts, edges, symptoms, nonEdges, references, walks, issues } =
    parseTree(root);
  ...
  issues.push(...validateContent(schema, concepts, edges, symptoms, references, walks, nonEdges));
  ...
  const graph = linkGraph(schema, concepts, edges, symptoms, nonEdges, references, walks, rendered);
